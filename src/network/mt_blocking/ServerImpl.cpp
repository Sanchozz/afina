...
void ServerImpl::OnRun() {
    ...
    while (running.load()) {
        ...
        {
            if (workers_count < workers_limit) {
                _logger->debug("Workers:{}\n", workers_count);
                std::thread(&ServerImpl::worker_handler, this, client_socket).detach();
                ++workers_count;
            } else {
                _logger->warn("Worker's limit reached{}\n", client_socket);
                close(client_socket);
            }
        }
    }
    // Cleanup on exit...
    std::unique_lock<std::mutex> lock_u(server_mutex);
    while (workers_count.load()) {
        cond_var.wait(lock_u);
    }
    _logger->warn("Network stopped");
}

void ServerImpl::worker_handler(int client_socket) {
    // Here is connection state
    // - parser: parse state of the stream
    // - command_to_execute: last command parsed out of stream
    // - arg_remains: how many bytes to read from stream to get command argument
    ...
    close(client_socket);
    std::lock_guard<std::mutex> lock_g(worker_mutex);
    --workers_count;
    cond_var.notify_one();
